...
module.exports = ({book, root, indent}) => {
  ...
  archive.append(
    xml('package', {xmlns: 'http://www.idpf.org/2007/opf', 'unique-identifier': 'uuid', version: '2.0'},
      ...
      h('manifest',
        ...
        book.coverURL && book.coverPage !== false ? h('item', {id: 'cover', 'media-type': 'application/xhtml+xml', href: 'text/_cover.xhtml'}) : [],
      ...
      h('spine', {toc: 'toc'},
        book.coverURL && book.coverPage !== false ? h('itemref', {idref: 'cover'}) : [],
        h('itemref', {idref: 'text-title'}),
        book.toc === false ? [] : h('itemref', {idref: 'nav'}),
        book.texts.map((text, i) =>
          h('itemref', {idref: `text-${i}`}))),
      h('guide',
        book.coverURL && book.coverPage !== false ? h('reference', {type: 'cover', title: 'Cover', href: 'text/_cover.xhtml'}) : [],
  ...
  if (book.coverURL && book.coverPage !== false) archive.append(
    ...
    xhtml({'xmlns:epub': NS_EPUB},
      ...
      h('body', {'epub:type': 'frontmatter'},
        ...
        h('nav', {'epub:type': 'landmarks', hidden: ''},
          h('h1', 'Guide'),
          h('ol',
            book.coverURL && book.coverPage !== false ? h('li', h('a', {'epub:type': 'cover', href: '_cover.xhtml'}, 'Cover')) : [],
